...
class ImportTraces extends \App\Controllers\BaseController {
	...
	public function index(){
		$data = array();
		$this->load->view('Generated/trace/importtraces_view', $data);
	}
	...
	private function parseFile($filename, $separator){
		...
		if (($handle = fopen($filename, "r")) !== FALSE) {
			while (($data = fgetcsv($handle, $maxSize, $separator)) !== FALSE) {
				...
				$model->dt_trace = $data[0];
				$model->app_id = $data[1];
				$model->target_id = $data[2];
				$model->detail = $data[3];	
				$model->save($this->db);
