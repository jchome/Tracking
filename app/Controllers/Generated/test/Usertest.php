...
class UserTest extends Toast {
	...
	public function test_update(){
		...
		if(!$this->_assert_equals($user_insert->email, $user_update->email)) {
			return false;
		}
	...
	function test_list(){
		...
		foreach ($users as $user) {
			if($user->id == $user_insert->id &&
					...
					$this->_assert_equals($user->email, $user_insert->email )
				){
				$found++;
			}
		}
